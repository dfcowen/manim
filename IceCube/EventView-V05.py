    ...
    def create_visualization(self, run, event):
        ...
        if hit_data:
            ...
            if z_coords:
                ...
                for string in hit_strings:
                    ...
                    for om in range(1, 61):  # Typical range of OMs per string
                        if (string, om) in self.module_positions:
                            x, y, _ = self.module_positions[(string, om)]
                            line = Line(
                                np.array([x, y, min_z]), 
                                np.array([x, y, max_z]), 
                                color=WHITE, 
            ...
            for (string, om), hits in hit_data.items():
                x, y, z = self.module_positions[(string, om)]
        ...
        text = Text(f"Run: {run}\nEvent: {event}")
        text.to_corner(UR)  # Changed from UL (Upper Left) to UR (Upper Right)
        ...
        # Create a small coordinate axes indicator
        axes_scale = 2.0  # Size of the axes
        axes_origin = np.array([-5, -5, -5])  # Position in bottom left
        
        # Create the axes lines
        x_axis = Arrow(axes_origin, axes_origin + np.array([axes_scale, 0, 0]), color=RED, buff=0)
        y_axis = Arrow(axes_origin, axes_origin + np.array([0, axes_scale, 0]), color=GREEN, buff=0)
        z_axis = Arrow(axes_origin, axes_origin + np.array([0, 0, axes_scale]), color=BLUE, buff=0)
        
        # Create the labels
        x_label = Text("x", color=RED).scale(0.5).move_to(axes_origin + np.array([axes_scale + 0.5, 0, 0]))
        y_label = Text("y", color=GREEN).scale(0.5).move_to(axes_origin + np.array([0, axes_scale + 0.5, 0]))
        z_label = Text("z", color=BLUE).scale(0.5).move_to(axes_origin + np.array([0, 0, axes_scale + 0.5]))
        ...
        self.camera.light_source.move_to(5*RIGHT + 5*IN + 5*UP)
        self.wait(0)
